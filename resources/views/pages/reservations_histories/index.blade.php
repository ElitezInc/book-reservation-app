        ...
        <div class="row">
            <div class="col">
                @if(request()->has('trashed'))
                    <a href="{{ route('admin.reservations_histories') }}" class="btn btn-info">View All</a>
                @else
                    <a href="{{ route('admin.reservations_histories', ['trashed' => true]) }}" class="btn btn-primary">View Trashed</a>
                @endif
                <hr />
            </div>
        </div>
    ...
    <script>
        $(function () {
            const table = $('.books_datatable').DataTable({
                ...
                @if(request()->has('trashed'))
                ajax: "{{ route('admin.reservations_histories', ['trashed' => true]) }}",
                @else
                ajax: "{{ route('admin.reservations_histories') }}",
                @endif
